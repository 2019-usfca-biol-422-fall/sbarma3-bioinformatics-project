...
ambiguity codes to rename bases. Once the sequences from all of my samples were trimmed, I used the NCBI
...
similar sequences found in the archive [@madden2013blast]. Using BLAST, I chose the results that were not unspecified, 
and had a high percentage match. Of the 3 samples that failed, I marked these files as 
failed sequences and did not use them for this project. I used MAFFT to create a multiple alignment sequence [@katoh2013mafft]. The setup for Mr.Bayes included having the substitution model as GTR, rate variation as invgamma,and 
outgroup set as *Thermus aquaticus*. The MCMC settings for chain length was set to 1,100,000, heated chains was set to 
4, heated chain temp was set to 0.2, subsampling freq set to 200, the burn-in length set to 
100,000, and the random seed length was set to 10,736. The rest of the settings were on default [@kearse2012geneious]. 
The setup for PhyML included having the substitution model as GTR, the branch support set 
as bootstrap and the number of bootstraps being 100. The rest of the settings 
were on default [@masters2011species]. Only 3 of my samples were run successfully and gave results. 
...
is understandable since they are both the same genus (posterior probability = 0.9976).
...
Out of my 6 original samples, only 3 came out as usable sequences. This probably resulted
from having a mixed template. It was difficult to identify isolated colonies, so 
it is highly likely that this was the reason half of my sequences were not usable. 
...
within the vicinity of the school campus, therefore I am not confident that this was 
identified correctly. However, two types of *Staphylococcus*
...
commonly on the skin or within the areas on campus, I am confident that these results were 
correctly identified. Both the phylogenetic results and BLAST results match up, which also
...
phones [@ulger2009we]. Although both professors and students had similar bacteria on their cellphone screens, 
...
(Figure 3 Bootstap = 100, Figure 4 posterior probability = 0.9976). Since both of the bacterial 
...
strains belong to the same clade as *Thermus aquaticus*. 
